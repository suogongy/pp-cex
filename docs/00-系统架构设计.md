...
graph TB
    ...
    class CoreGateway,Route,LoadBalance,CircuitBreaker,Auth gatewayCore
...
graph TB
    ...
    AccessExample[访问层]
    GatewayExample[网关层]
    GatewayCoreExample[网关核心功能]
    GatewayDepExample[网关外部依赖]
    MessageExample[消息层]
    CacheExample[缓存层]
    DataExample[存储层]
    InfraExample[基础设施层]
    ...
    class GatewayCoreExample gatewayCore
    class GatewayDepExample gatewayDependency
